...
\runtitle{Rao-Blackwellised Particle Smoothing}
...
The various Rao-Blackwellised smoothing algorithms have been tested on a number of models with a variety of linear and nonlinear dynamics. Here we present an illustrative example.

\comment{
Which model shall I put in? What do we actually want to demonstrate with these tests?
I currently have results which demonstrate the following:
\begin{itemize}
  \item All the backward simulation smoothers (BSSs) outperform the filter/smoother.
  \item The Kim's approximation BSS is not as good as the exact forms.
  \item The linear sampling and fully RB BSSs produce virtually identical results.
\end{itemize}
What we'd like to be able to show, but can't (yet):
\begin{itemize}
  \item Situations in which the fully RB BSS outperforms the linear sampling version.
  \item How the reweighting smoother compares.
  \item How the Briers \& co two-filter smoother compares.
\end{itemize}
}
